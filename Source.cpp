...
int main() {
	...
	srand(time(0));
	Tackt = -1;
	int p = 1;//1 + std::rand() % 5;
	int q = 1;//1 + std::rand() % 5;
	int m = 2;//1 + std::rand() % 5;
...
double avb(double a, double b) {
	...
	consolemut.lock();
	std::cout << "Вызван процессорный элемент считающий функцию а -> b" << std::endl;
	consolemut.unlock();
	int time = Tackt;
	while (time + 1 > Tackt) {
...
double bva(double a, double b) {
	...
	consolemut.lock();
	std::cout << "Вызван процессорный элемент считающий функцию b -> a" << std::endl;
	consolemut.unlock();
	int time = Tackt;
	while (time + 1 > Tackt) {
		std::this_thread::sleep_for(std::chrono::milliseconds(1));
...
double aorb(double a, double b) {
	...
	consolemut.lock();
	std::cout << "Вызван процессорный элемент считающий функцию a ^ b " << std::endl;
	consolemut.unlock();
	int time = Tackt;
	while (time + 1 > Tackt) {
		std::this_thread::sleep_for(std::chrono::milliseconds(1));
